...
module "sol_infra_sg" {
  ...
  vpc_id       = data.aws_vpc.vpc_info.id
  ingress_list = concat(local.infra_lb_subnet_cidr_ingress, local.private_subnet_cidr_ingress, local.nat_ip_sg_rules_ingress, local.jumpbox_sg_rules_ingress, local.ansible_ip_rules_ingress)
  egress_list  = local.default_sg_rules_egress
  vpc_name     = var.vpc_name
  system_type  = var.system_type
}
...
module "sol_master_sg" {
  ...
  vpc_id       = data.aws_vpc.vpc_info.id
  ingress_list = concat(local.k8s_lb_subnet_cidr_ingress, local.private_subnet_cidr_ingress, local.nat_ip_sg_rules_ingress, local.ansible_ip_rules_ingress, local.jumpbox_sg_rules_ingress)
  egress_list  = local.default_sg_rules_egress
  vpc_name     = var.vpc_name
  system_type  = var.system_type
}
