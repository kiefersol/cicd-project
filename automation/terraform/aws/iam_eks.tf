...
module "k8s-cluster-AmazonEKSClusterPolicy" {
  ...
  role_name  = module.sol_eks_role[0].iam_role_info.name
}
...
# Node Group
module "sol_nodegroup_role" {
  source           = "./modules/iam/role"
  count            = var.k8s_eks_install ? 1 : 0
  role_name        = join("-", ["${var.vpc_name}", "${var.system_type}", "worker", "nodegroup_role"])
  role_policy_file = "./templates/iam/role_ec2.tpl"
}

module "sol_nodegroup_policy" {
  source      = "./modules/iam/policy"
  count       = var.k8s_eks_install ? 1 : 0
  policy_name = join("-", ["${var.vpc_name}", "${var.system_type}", "worker", "nodegroup_policy"])
  policy_file = "./templates/iam/policy_nodegroup.tpl"
}
...
module "k8s-worker-AmazonEKSWorkerNodePolicy" {
  ...
  role_name  = module.sol_nodegroup_role[0].iam_role_info.name
}
...
module "k8s-worker-AmazonAutoScailingPolicy" {
  source     = "./modules/iam/policy_attachment"
  count      = var.k8s_eks_install ? 1 : 0
  policy_arn = module.sol_nodegroup_policy[0].iam_policy_info.arn
  role_name  = module.sol_nodegroup_role[0].iam_role_info.name
}
